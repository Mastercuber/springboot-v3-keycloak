package de.avensio.keycloak.security;
...
public class KeycloakOidcUserService implements OAuth2UserService<OidcUserRequest, OidcUser> {
    ...
    public OidcUser loadUser(OidcUserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        for (String role : ((LinkedHashMap<String, Collection<String>>)userInfo.getClaims().get("realm_access")).get("roles")) {
            authorities.add(new SimpleGrantedAuthority(
                role.replace("REALM_ROLE_", "ROLE_")
                    .replace("CLIENT_ROLE_", "ROLE_").toUpperCase()
            ));
        }
